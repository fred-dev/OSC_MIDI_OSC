...
SettingsManager::~SettingsManager() {
    // Destructor code
    ofLogNotice("SettingsManager") << "Destructor called";
    
    
}

SettingsManager& SettingsManager::getInstance() {
    static SettingsManager instance; // Guaranteed to be destroyed and instantiated on first use
    return instance;
}
    ...
    if (ofFile::doesFileExist(filename)) {
        ...
        if (!settings["midiInDevice"].is_null()) {
            if (settings["midiInDevice"].is_number()) {
                ...
                ofLogVerbose("loadSettings") << "midiInDevice set to string: " << settings["midiInDevice"] << endl;
            }
        } else {
            settings["midiInDeviceByString"] = false;
        ...
        if (!settings["midiOutDevice"].is_null()) {
            if (settings["midiOutDevice"].is_number()) {
                ...
                ofLogVerbose("loadSettings") << "midiOutDevice set to number: " << midiOutDeviceNum << endl;
            } else if (settings["midiOutDevice"].is_string()) {
                settings["midiOutDeviceByString"] = true;
                ofLogVerbose("loadSettings") << "midiOutDevice set to string: " << midiOutDeviceName << endl;
            }
        } else {
            settings["midiOutDeviceByString"] = false;
